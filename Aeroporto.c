...
int intervalo_min = 1000; // Intervalo mínimo em milissegundos (1s)
int intervalo_max = 3000; // Intervalo máximo em milissegundos (3s)
...
void* monitor_thread(void* arg __attribute__((unused))) {
    while (simulation_running) {
        ...
        printf("Alertas: %d | Starvation: %d | Deadlocks: %d | Preempções: %d\n", 
               alertas_criticos, starvation_casos, deadlocks_detectados, preempcoes_realizadas);
...
int force_preemption(int critical_aviao_id) {
    ...
    for (int i = 0; i < airplane_counter; i++) {
        if (avioes[i].type == VOO_INTERNACIONAL && 
            ...
            release_res(&torre, VOO_INTERNACIONAL, 1);
            release_res(&pistas, VOO_INTERNACIONAL, 0);
            release_res(&portoes, VOO_INTERNACIONAL, 0);
...
int main(int argc, char *argv[]) {
    ...
    pthread_t monitor_tid, aging_tid;
